...
// Helper Functions ----------------------------------------
// Phase 4: the SNMP / MIB-II TCP connection-table walk that used to
// populate the local chat IP was Windows-only (inetmib1.dll + snmpapi.dll
// LoadLibrary'd at runtime). Replaced by a portable UDP connect() +
// getsockname() in GameNetwork/GameSpy/LocalChatAddress.cpp.
