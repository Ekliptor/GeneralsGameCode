...
**Root cause (downstream of the CSF fix).** The font pipeline is not wired for the BGFX build:gain
...
## Shell-map scene-population — round 2 update (2026-04-26)

After Phase 0/A/B/C un-stubbed the W3D→bgfx path (RenderObjClass, WW3D::Render entry points, shader-manager pass count, mesh PolygonRendererList guards), `RTS3DScene::Customized_Render` was firing every frame but `UpdateList=0 RenderList=0` — the scene was empty.

**Root cause:** `Generals/Code/GameEngine/Source/Common/GameLOD.cpp:625-636` and `GeneralsMD/Code/GameEngine/Source/Common/GameLOD.cpp:631-641` had an explicit `#ifndef RTS_RENDERER_DX8` block that force-disabled `TheWritableGlobalData->m_shellMapOn = false`. That gate was added in earlier bgfx work as a "render-path-not-yet-ported" workaround. With it active, `Shell::showShellMap(TRUE)` took the `BlankWindow.wnd` fallback in the `if(useShellMap && TheGlobalData->m_shellMapOn)` branch, never posting `MSG_NEW_GAME GAME_SHELL`, so `W3DTerrainVisual::load("Maps\ShellMapMD\ShellMapMD.map")` was never called and `m_3DScene->Add_Render_Object(m_terrainRenderObject)` never fired.

**Fix:** removed the gate in both targets. Verified via temporary trace:
- before: `m_shellMapOn=0`, no W3DTerrainVisual::load, Customized_Render UpdateList=0 RenderList=0
- after: `m_shellMapOn=1`, ShellMapMD.map loads, Add_Render_Object fires, Customized_Render UpdateList=1→5 over time, RenderList=2 (terrain + water).

**Secondary regression — particle crash:** with terrain populated, `terrainObject != nullptr` enables the call to `TheParticleSystemManager->queueParticleRender()` at the end of Customized_Render. That hits `PointGroupClass::Update_Arrays + 6272` with a NULL read (`PointGroupClass::_QuadVertexLocationOrientationTable` lookup, `__sincosf_stret`-adjacent code). Particles are Phase D scope and not yet ported. Gated the queue call site under `#ifdef RTS_RENDERER_DX8` in both `{Generals,GeneralsMD}/.../W3DScene.cpp::RTS3DScene::Customized_Render` to defer.

**Outstanding:** terrain/water `Render` is now invoked but the visible 3D area of the menu remains dark. Likely causes (next investigation):
1. View matrix not repopulated for 3D after Render2D leaves it identity (per Phase A risk #2).
2. Lighting environment empty in BGFX, terrain self-lit but with zero ambient.
3. `HeightMapRenderObjClass::Render` early-out we haven't found yet (BGFX-incompatible state).

The Phase A/B/C wiring is now end-to-end testable for the menu shell map. Phase D (particles, water effects) and the dark-render diagnosis are the remaining items before the menu shows the ZH tank-battle backdrop.

## Phase D part 1 — matrix layout fixes (2026-04-26)

Two upstream bugs that were silently turning every 3D submit into a no-op:

### `Set_Projection_Transform_With_Z_Bias` never published to bgfx

`Core/.../dx8wrapper.h::Set_Projection_Transform_With_Z_Bias` (called every frame from `CameraClass::Apply`) only fired a `DX8CALL(SetTransform(D3DTS_PROJECTION,…))`. In bgfx mode `DX8CALL` is `((void)0)`, so the projection matrix never reached `render_state.projection` or set the `PROJECTION_CHANGED` dirty bit. `Apply_Render_State_Changes` therefore never drained a real projection to the bgfx backend — every draw used identity. Fix: also write `render_state.projection = matrix; render_state_changed |= PROJECTION_CHANGED;` so the bgfx drain works (the `Set_Transform(D3DTS_PROJECTION, m)` overload at line 1265 already did this; this just brings the z-bias variant into parity).

Diagnostic that surfaced this: per-call trace in `BgfxBackend::Set_View_Transform` / `Set_Projection_Transform`. First 2880 calls were all identity; non-identity values only arrived once we set the dirty bit.

### EA's Matrix4x4 row-major vs bgfx column-major

`Apply_Render_State_Changes` was passing `render_state.world/view/projection` to `b->Set_*_Transform` via `reinterpret_cast<const float*>(&m)`. EA's `Matrix4x4` stores `Row[4]` of `Vector4` (row-major; translation in `Row[0..2][3]`, i.e. linear indices m[3], m[7], m[11]). bgfx — like D3D — expects column-major (translation at m[12..14]). The DX8 path correctly transposed via `To_D3DMATRIX` (`matrix4.cpp:203` does `dxm.m[i][j] = m[j][i]`); the bgfx path skipped that step.

Without the transpose, every 3D submit had its view/projection/world interpreted as the *transpose* of the intended matrix — geometry projected to a degenerate region, nothing visible. Fix: a 4×4 transpose lambda in `Apply_Render_State_Changes` before each `Set_*_Transform` call, matching what `To_D3DMATRIX` does for DX8.

Diagnostic that surfaced this: traced terrain submits in `BgfxBackend::Draw_Triangles_Dynamic` (4096-vert batches = `HEIGHTMAP_VERTEX_NUM`). Translation showed up at m[3]/m[7]/m[11] (row-major) instead of m[12]/m[13]/m[14] (column-major) — instant tell.

### Net result

Build clean, no crash, no regression to the menu UI. Terrain still doesn't produce visible pixels (separate Phase D part 2: probably texture binding, terrain shader-program selection, or fragment output), but the matrix pipeline is now correct end-to-end and verified via per-draw trace.

### Phase D part 2 — what to investigate next

Diagnostic showed: terrain submits are reaching `bgfx::submit` with proper view/proj/world. Forced clear-color to magenta showed the 3D area gets cleared by `W3D::Render(scene_3d, cam, clear=true, clear_color=black)`, and terrain submits don't overdraw. So terrain output is being dropped after submit. Candidates:
1. Vertex layout / FVF mismatch — terrain stride=32, multi-UV; uber-shader may not bind right attributes.
2. Texture stage 0 not bound for terrain (the W3D shader manager `setTexture` calls in `HeightMap.cpp:1989-1992` go through `W3DShaderManager::setTexture` which we BGFX-gated to no-op in Phase B).
3. The selected uber-shader program (solid/vcolor/tex/tex_mlit/tex2) doesn't match terrain's vertex layout, so `bgfx::ProgramHandle` is invalid and the submit is silently dropped.
4. Backface cull — terrain triangles may have winding that gets culled with bgfx's default LH/RH convention.

Recommended next diagnostic: in `BgfxBackend::Draw_Triangles_Dynamic`, log `prog.idx` and `m_stageTexture[0..3]` for 4096-vert submits — if program is invalid or texture stage 0 is `0`, that's the lead.

## Phase D part 2 — landed (2026-04-27)

Two upstream fixes landed; the third root cause was uncovered during verification and is queued as Phase D part 3.

### Fix 1 — CW culling state propagated to bgfx

`Core/GameEngineDevice/Source/BGFXDevice/Common/BgfxBackend.cpp:362 BuildStateMask` had the cull-line commented out (debug holdover). Re-enabled with `BGFX_STATE_CULL_CW`:
...
Verified by toggling: CCW eliminates 2D UI, CW preserves it. So 2D geometry is CW-wound and matches D3D's clockwise=front convention. CW cull is correct for the W3D pipeline.

### Fix 2 — terrain stage-0 texture binding in BGFX

`Core/.../W3DShaderManager.cpp:2724 setShader` BGFX path was `return TRUE;` (Phase B). The DX8 multipass setShader-per-pass path called `DX8Wrapper::Set_Texture(0, m_Textures[0])` to bind the terrain diffuse base texture. Without that call, `BgfxBackend::m_stageTexture[0]` stayed null and bgfx fell back to the 2×2 white placeholder. Fix: BGFX path now calls `DX8Wrapper::Set_Texture(0, m_Textures[0])` so the diffuse base reaches the backend; cloud / lightmap / noise stages stay deliberately unbound (Phase D part 3 territory). Verified by trace: `[D2-trace] terrain submit prog=11 tex0=53822215472` — non-zero texture handle bound to stage 0 program 11 (m_progTex).

### Phase D part 3 (next phase) — terrain texture content + UV computation are still DX8-bound

After fixes 1 and 2, terrain still renders black. The diagnostic that surfaced the actual blocker:

```
[D2-trace] v0 pos=180.0,-250.0,0.0 uv0=0.213,inf uv1=0.213,inf
[D2-trace] v1 pos=190.0,-250.0,17.5 uv0=0.229,inf uv1=0.229,inf
```

**Every terrain vertex's V coordinate is `+inf`.** Root cause: `WorldHeightMap::getUVForTileIndex` divides by `m_terrainTexHeight`, which is set from the return value of `TerrainTextureClass::update` (`Core/.../TerrainTex.cpp:95`). That function is a wholly DX8-surface-lock-based texture-blitter:
...
if (surface_desc.Width < TEXTURE_WIDTH) {
    ...
    return 0;   // ← returns 0 in bgfx
}
```

`Peek_D3D_Texture()` returns nullptr in BGFX (no DX8 device), the `DX8_ErrorCode` macro is a no-op in non-debug, the early-return at `Width < TEXTURE_WIDTH` triggers (or worse, the path crashes silently). End state: `m_terrainTexHeight = 0`, **and** the actual texture pixels are never blitted in. So even if the V/0 division were guarded, the bound terrain texture is empty/garbage.

**Phase D part 3 scope** — port `TerrainTextureClass::update` (and the sibling `AlphaTerrainTextureClass`, `AlphaEdgeTextureClass`) to a BGFX-friendly path. Options:
- **(a)** Build the terrain mosaic in a CPU staging buffer (`std::vector<uint16_t>` for A1R5G5B5 or `uint32_t` for ARGB8) using the existing tile-blit logic with the DX8 surface APIs swapped for plain-pointer arithmetic, then call `bgfx::updateTexture2D` (or `IRenderBackend::Create_Texture_RGBA8` once at construction).
- **(b)** Skip the per-tile mosaic and have the bgfx backend sample tiles directly from a texture atlas built once in CPU. Larger refactor.

Option (a) is the lower-risk path: the inner loop at `TerrainTex.cpp:131-152` already operates on byte pointers — only the `LockRect` + `surface_desc.Width` plumbing needs replacing with a CPU surface descriptor. Output `m_terrainTexHeight` becomes the real height, V coords compute cleanly, and the bgfx-side texture handle (already bound at stage 0 by Phase D part 2's fix 2) gets real content.

`AlphaTerrainTextureClass` and `AlphaEdgeTextureClass` follow the same pattern; they're shrouded under cloud / lightmap / noise stages that are Phase D part 4 anyway, so they can stay no-op for part 3.

## Phase D part 3 — landed (2026-04-27)

### Fix — TerrainTex.cpp / AlphaEdge ported to BGFX-friendly path

`Core/.../TerrainTex.cpp` `TerrainTextureClass::update` and `AlphaEdgeTextureClass::update` now have `#ifndef RTS_RENDERER_DX8` paths that:

- Read `Get_Width()` / `Get_Height()` from the texture (set by the `TextureClass(width, height, ...)` ctor at `texture.cpp:526` which already allocates an empty bgfx handle via `Create_Texture_RGBA8(nullptr, w, h, mipmap)`).
- Allocate a `width × height × 4` byte CPU staging buffer in RGBA8.
- Walk the same tile-blit logic the DX8 path uses, but swap the destination pointer math from `IDirect3DSurface8::LockRect` output to plain pointer arithmetic in the staging buffer. Pixel format conversion: source is BGRA, destination is RGBA8 (R = pBGR[2], G = pBGR[1], B = pBGR[0], A = 255 for terrain / alpha-from-source-mask for AlphaEdge).
- Replicate the 4-pixel border duplication around each tile class (matches DX8 path).
- Upload via `IRenderBackend::Update_Texture_RGBA8(handle, staging, w, h)` and return `height` (so `m_terrainTexHeight` is real, no longer 0 → V coords compute cleanly).

`updateFlat` and `setLOD` left under DX8-only — they hit the same `Peek_D3D_Texture()` paths but are gated to terrain-edit / runtime-LOD codepaths that aren't reached in the menu / skirmish boot path.

Verification trace confirmed Phase D part 2 + part 3 both healthy:

```
[PhaseD3] TerrainTextureClass alloc w=2048 h=1024 update=1024 bgfxHandle=52782449880
[PhaseD3-draw] tri verts=4096 idx=6144 attrMask=0x1d prog=11 tex0=52782449880 cull=1 depth=3 alpha=0
```

### `-skirmish [<map>]` CLI added

For in-game (not menu shellmap) verification with screenshots. Implementation:

- `GeneralsMD/Code/GameEngine/Source/Common/CommandLine.cpp::parseSkirmish` — sets `m_autoSkirmish=TRUE`, skips intros, optionally captures explicit map argument.
- New file `GeneralsMD/Code/GameEngine/Source/GameClient/GUI/GUICallbacks/Menus/AutoSkirmish.cpp::TryAutoSkirmishLaunch` — replicates the SkirmishGameOptionsMenu's Start-button flow programmatically: constructs `TheSkirmishGameInfo`, configures slot 0 (local human, faction/color from `SkirmishPreferences`) and slot 1 (Easy AI), picks the map (explicit arg → preferred → first multiplayer in cache), appends `MSG_NEW_GAME(GAME_SKIRMISH)`.
- Hook in `GameClient::update` calls `TryAutoSkirmishLaunch` once the shell is active. The screenshot countdown is bumped to 1500 frames (~50 s) to allow map load + first-frame settle. The `-screenshot` capture path is updated to also trigger when `TheGameLogic->isInGame()` (not just `TheShell->isShellActive()`).

Use:
```
scripts/run-game.sh --target zh -- -win -skirmish -screenshot /tmp/skirmish-zh.tga
```

Verified: game loads "Alpine Assault" map, 1v1 vs Easy AI, HUD/money/timer all visible. Screenshot lands on the gameplay scene, not the menu.

### Discovery — bgfx setViewTransform is per-VIEW, not per-submit (architectural blocker)

Even with Phase D parts 1/2/3 all landed, the 3D area still renders black. Diagnostic: forced the bgfx clear color to magenta — the 3D area stays magenta in both the menu shellmap and skirmish gameplay. So terrain submits **don't produce visible fragments**, while 2D UI overlays (HUD, money, buttons) on top of the magenta render correctly.

A trace at `BgfxBackend::End_Scene` (right before `bgfx::frame`) of the cached `m_viewMtx` / `m_projMtx` (which is what bgfx will apply via the most-recent `setViewTransform(0, ...)` call) showed:

```
[PhaseD3-frame] last view IDENTITY tr=(0.0, 0.0, 0.0)  proj m[0]=1.0000 m[5]=1.0000
```

Every frame ends with view-0 transforms set to identity. Then `bgfx::frame()` applies that identity to **all submits** in view 0 — including terrain submits earlier in the frame that called `setViewTransform(0, viewMatrix, projMatrix)` with the real camera transforms.

This is a fundamental architectural mismatch between the DX8 model (per-call `SetTransform(D3DTS_VIEW, m)` is mutable state that takes effect for the next draw) and bgfx (`setViewTransform(viewId, ...)` updates the view's transform, but **the LAST call before `bgfx::frame()` wins for ALL submits to that view**).

Both 3D world geometry (terrain, units) and 2D UI (Render2D, font glyphs) currently submit to view 0 with different view+proj combinations. The 2D submit path runs LAST per frame and overwrites the 3D camera with identity → 3D submits get the wrong projection → terrain coords (~hundreds of units) clip outside `[-1,1]` NDC → no fragments visible.

**Phase D part 4 scope** — split bgfx views by render-pass intent:

- **View 0:** 3D world. View transform from camera. Perspective projection. Depth test enabled.
- **View 1:** 2D UI. Identity view + ortho projection (or no setViewTransform at all — Render2D submits already produce in-NDC quads). No depth test.

Implementation needs:
1. `BgfxBackend` to track a per-frame "logical view bucket" and route `Draw_Triangles_Dynamic` / `Draw_Indexed` submits to the appropriate bgfx view ID. Selection: probably driven by `m_shader.depthCmp != DEPTH_ALWAYS` (3D) vs identity/ortho-equivalent submits (2D).
2. `bgfx::setViewMode(viewId, bgfx::ViewMode::Sequential)` for both so submission order is preserved.
3. `bgfx::setViewClear` only on view 0 (3D); view 1 inherits the framebuffer.
4. `bgfx::setViewOrder` so view 0 (3D) submits before view 1 (2D).

The cleanest seam is the call site: `Draw_Triangles_Dynamic` examines the projection matrix's `m[15]` (1.0 for ortho, 0.0 for perspective) or the cull/depth state and picks the view ID. Alternative: introduce a `IRenderBackend::Begin_2D_Pass()` / `End_2D_Pass()` method the 2D layer calls, and route those to a different view inside bgfx.

Without this fix, terrain (and HLOD meshes once Phase C v2 lands) cannot become visible. Phase D parts 1-3 are real upstream bugs that needed fixing regardless — they're prerequisites — but the visibility gate is bgfx view-routing.

## Phase D part 4 — landed (2026-04-27)

Implemented in `Core/GameEngineDevice/Source/BGFXDevice/Common/BgfxBackend.cpp` and the matching header. Reserved `kView3D=0` for the backbuffer 3D scene and `kView2D=1` for the HUD/menu overlay; bumped `m_nextViewId` initial from 1→2 so render targets allocate from view ID 2.

Key design:
- **Routing key**: `Set_Projection_Transform` latches `m_active2D = (fabsf(proj[15] - 1.0f) < 1e-4f)` — perspective (`proj[15]==0`) routes to kView3D, ortho/identity routes to kView2D. Rejected `s.cullEnable` because legitimate 3D content (foliage, billboards, water, decals) uses CULL_MODE_DISABLE.
- **Latch ordering safe**: `dx8wrapper.cpp::Apply_Render_State_Changes` and `Render2DClass::Render` both push WORLD→VIEW→PROJECTION, so the latch is current by the time `ApplyDrawState` consumes it.
- **Mode-flip cross-copy**: when `Set_Projection_Transform` flips the mode, the just-stored VIEW lives in the previous mode's slot (since VIEW wrote first). Retroactively memcpy it into the new mode's slot so the next submit uses paired view+proj.
- **RT gate**: when `m_currentView != 0` an RT is bound, and the split is bypassed — RTs own their view ID; 3D and 2D both target the RT view.
- **Clear / touch / view order**: kView3D owns the backbuffer color/depth clear (`Clear()` redirects to kView3D when no RT). `Begin_Scene` touches both backbuffer views so they always render. `UpdateViewOrder` extends the explicit order to `[RTs..., kView3D, kView2D]` so 2D HUD draws over 3D. kView2D set to `ViewMode::Sequential` for submission-order layering and `BGFX_CLEAR_NONE` so it alpha-blends over 3D.

Verified with screenshots:
- ZH menu (`-screenshot /tmp/phaseD4-menu.tga`) — terrain hull (snowy mountain landscape) visible behind menu UI.
- ZH skirmish (`-skirmish -screenshot ...`) — green grass terrain fills the upper area; HUD (top bar, money, build menu, sidebar) correctly layered on top. This was the failing case in `generals-black.png` (pure black 3D area).
- Vanilla menu — paved cobblestone + grass patches + water; Generals logo overlays correctly.

The terrain texture appears blocky/quilted in some areas — Phase D part 5+ scope (cloud / lightmap / noise multipass shaders deferred from D2). HLOD units / buildings still need Phase C v2.

## Phase D part 5 — landed (2026-04-27)

Implemented in:
- `Core/GameEngineDevice/Source/BGFXDevice/Shaders/{vs,fs}_terrain.sc` — new shader pair.
- `Core/GameEngineDevice/Source/BGFXDevice/CMakeLists.txt` — `terrain` added to BGFX_SHADER_PAIRS.
- `Core/GameEngineDevice/Source/BGFXDevice/Common/BgfxBackend.cpp` + matching header — `m_progTerrain`, `m_terrainPassActive` flag, `Set_Terrain_Pass_Active()` override, embedded shader includes, init/destroy, SelectProgram routing, stage-1 binding for terrain prog.
- `Core/Libraries/Source/WWVegas/WW3D2/IRenderBackend.h` — virtual no-op `Set_Terrain_Pass_Active(bool)` so non-bgfx backends ignore the hint.
- `Core/GameEngineDevice/Source/W3DDevice/GameClient/W3DShaderManager.cpp` — BGFX gate in setShader/resetShader binds stage 1 to `m_Textures[1]` and toggles the backend flag for `ST_TERRAIN_BASE` / `_NOISE1` / `_NOISE2` / `_NOISE12`.

Key design:
- **Single-pass fold of the DX8 multipass terrain shader.** Fixed-function passes 0+1 (base diffuse with vColor modulate, then alpha-edge framebuffer blend) collapse into one fragment that samples both stages and computes `mix(base.rgb, edge.rgb, edge.a * v_color0.a) * v_color0.rgb`. Output alpha forced to 1.0 (terrain is opaque). Cloud / lightmap pass 2 stays deferred to Phase D6.
- **Critical: vColor.a is the per-vertex edge-blend mask.** `BaseHeightMapRenderObjClass::doTheLight` (BaseHeightMap.cpp:602) packs the `alpha[i]` value from `WorldHeightMap::getAlphaUVData` into the diffuse alpha channel. Tiles with no edge transition have vColor.a == 0 → mix factor 0 → base shows through. First implementation used `mix(base, edge, edge.a)` and got large white squares wherever the atlas's UV1 region had `edge.a == 1, edge.rgb == white`; multiplying by `v_color0.a` is what makes the alpha-edge atlas's "no transition" regions invisible.
- **Routing through a backend hint, not vertex layout.** SelectProgram already handles tex / tex2 / texMLit by attribute presence, but terrain has the same layout as tex2 (XYZ + diffuse + UV0 + UV1) and would otherwise resolve to the modulate shader. `Set_Terrain_Pass_Active(true)` from W3DShaderManager around setShader / setTexture(stage 1) gives SelectProgram a one-bit override; the flag clears on resetShader so follow-on draws (bridges, scorches, units once they land) don't accidentally route through the terrain program.
- **Stage 1 binding gate extended.** ApplyDrawState's `needsStage1` predicate now matches both `m_progTex2` and `m_progTerrain` so bgfx's sampler validation doesn't warn when terrain is bound (the other programs don't declare `s_texture1`).

Verified with screenshots:
- Vanilla menu (`phaseD5-vv.png`) — paved cobblestone smoothly transitions into grass tiles. Edge regions blend correctly; tile boundaries show natural gradients instead of hard atlas edges. Generals logo overlays correctly.
- ZH skirmish (`phaseD5-zh-skirmish.png`) — green grass terrain with subtle texture variation across the field. HUD layered correctly (top bar, money, build menu, sidebar, healthbar).
- ZH menu (`phaseD5b-zh-menu.png`) — terrain texture is detailed (rocky/dirt with edge variation). The white blocks visible in the upper-left are unported HLOD building props from the shellmap (Phase C v2 scope), not terrain artifacts. The terrain itself renders cleanly.

Remaining issues:
- Cloud / lightmap multipass overlay (DX8 `TerrainShader2Stage::set` pass 2) — Phase D6+. Requires camera-space-position UV gen + per-frame cloud animation offset + multiply blend over the terrain. Unlike the alpha-edge pass, this benefits from procedural UVs computed in the vertex shader from world position, since the legacy DX8 path used `D3DTSS_TCI_CAMERASPACEPOSITION + D3DTTFF_COUNT2` which has no direct shader equivalent.
- HLOD unit / building draws — Phase C v2 (the white blocks in ZH menu are still untextured / unrendered HLOD).
