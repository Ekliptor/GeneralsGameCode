/*
**	Phase 5i smoke test: real RGBA8 texture upload through IRenderBackend.
**
**	Same cube geometry as tst_bgfx_mesh, but with a procedurally-generated
**	16×16 RGBA8 checkerboard uploaded via Create_Texture_RGBA8 and bound
**	via Set_Texture(0, handle). A successful run proves the backend
**	correctly owns + binds caller-supplied textures and that the
**	`tex_lit` uber program samples them per-face. Exits after ~3s with
**	`tst_bgfx_texture: PASSED`.
**
**	Also verifies Destroy_Texture by releasing the handle before Shutdown
**	and confirming no bgfx validation errors.
*/
...
// Same cube layout as tst_bgfx_mesh but with per-face white color0 so the
// albedo contribution comes from the texture, not the vertex color.
...
// 16×16 procedural RGBA8: 4×4 blocks alternating orange / teal, with a
// diagonal white stripe so mirror/rotation bugs are obvious at a glance.
void MakeCheckerboard(uint8_t out[16*16*4])
{
	for (int y = 0; y < 16; ++y)
	for (int x = 0; x < 16; ++x)
	{
...
int main(int /*argc*/, char* /*argv*/[])
{
	...
	SDL_Window* window = SDL_CreateWindow("tst_bgfx_texture", kW, kH, SDL_WINDOW_RESIZABLE);
	...
	// Upload the checkerboard.
	uint8_t pixels[16*16*4];
	MakeCheckerboard(pixels);
	...
	SDL_Quit();
	fprintf(stderr, "tst_bgfx_texture: PASSED\n");
	return 0;
}
