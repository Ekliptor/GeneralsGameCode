/*
**	Phase 5n smoke test: shader-side alpha cutout via u_alphaRef.
**
**	Same cube as tst_bgfx_texture, but the 16×16 texture is now 50%
**	transparent on a 4×4-block checkerboard — half the texels are fully
**	opaque with alpha=255, half are alpha=0. ShaderStateDesc::alphaTest
**	is on with alphaTestRef=0.5, so the transparent texels are `discard`-ed
**	in the fragment shader and you see holes punched through every face of
**	the spinning cube (depth buffer updated per fragment, so the opaque
**	bits correctly occlude anything behind them — including the back face
**	of the cube itself, which is visible through the holes).
**
**	Exits after ~3s with `tst_bgfx_alphatest: PASSED`.
*/
...
// 16×16 RGBA8: every 4×4 block is either fully opaque orange or fully
// transparent (alpha=0). With alphaTestRef=0.5, the alpha=0 blocks are
// `discard`-ed, punching holes in each face.
void MakeAlphaCheckerboard(uint8_t out[16*16*4])
{
	for (int y = 0; y < 16; ++y)
	for (int x = 0; x < 16; ++x)
	{
		const int bx = x / 4;
		const int by = y / 4;
		const bool isOpaque = ((bx + by) & 1) == 0;
		uint8_t* p = &out[(y*16 + x) * 4];
		if (isOpaque) { p[0] = 0xE8; p[1] = 0x80; p[2] = 0x28; p[3] = 0xFF; }
...
int main(int /*argc*/, char* /*argv*/[])
{
	...
	SDL_Window* window = SDL_CreateWindow("tst_bgfx_alphatest", kW, kH, SDL_WINDOW_RESIZABLE);
	...
	shader.cullEnable      = false;   // show back faces through the holes
	shader.alphaTest       = true;    // Phase 5n — the cutout
	shader.alphaTestRef    = 0.5f;
	...
	SDL_Quit();
	fprintf(stderr, "tst_bgfx_alphatest: PASSED\n");
	return 0;
}
